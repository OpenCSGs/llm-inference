...
def query(
    ...
    if _is_llmserve_model(model):
        ...
        if appname in serve_details.applications.keys():
            if (
                _reverse_prefix(model)
                in serve_details.applications[appname].dict().get("deployments").keys()
            ):
                route_prefix = (
                    serve_details.applications[appname].dict().get(
                        "route_prefix")
                )
            else:
                raise Exception(
                    f"No {model} deployed under application {appname}. Ensure specify reasonable application and model name."
                )
        else:
            raise Exception(
                f"Specify reasonable application name for the model {model}."
            )
...
def batch_query(
    ...
    if _is_llmserve_model(model):
        ...
        if _supports_batching(model):
            ...
            result = [{"generated_text": llm.predict(
...
def run_experimental(
    model: str, appname: str = None, port: int = DEFAULT_HTTP_PORT
) -> None:
    """Run LLMServe on the local ray cluster
    ...
    for app in appname:
        ...
        if app in serve_details.applications.keys():
            ...
            serving_status[app] = {
                ...
                "deployments_status": deployment_status,
            }
